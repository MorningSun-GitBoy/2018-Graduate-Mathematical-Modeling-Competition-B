...
% p: 输出最佳个体出现迭代次数
% q: 输出最佳个体自变量值
function [m,n,p,q,path_all] = genetic_algorithm(population_size, chromosome_size, generation_size, cross_rate, mutate_rate, elitism)
...
[~,~,citypairs_num,pop_citypairs,gdp_citypairs,dis_citypairs]=citypairs_function(city,num_population,coordinate,GDP);

init(population_size); % 初始化，随机产生01矩阵

for G=1:generation_size   
%     fitness(population_size,chromosome_size,matrix_flow_adjust,matrix_ca,node_ca,matrix_man_cross_dis);              % 计算适应度 
%     fitness(population_size, chromosome_size,pop_citypairs,dis_citypairs)
    fitness(population_size, chromosome_size,citypairs_num,pop_citypairs,gdp_citypairs,dis_citypairs)
