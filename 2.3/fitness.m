...
function fitness(population_size, chromosome_size,citypairs_num,pop_citypairs,gdp_citypairs,dis_citypairs)
    ...
    if punish1==1||punish2==1
        for j=1:chromosome_size %不满足条件进行惩罚计算
            if population(i,j) == 1
                ...
                fitness_value(i)=fitness_value(i)+(sqrt(gdp_citypairs(citypairs_num(j,1))*gdp_citypairs(citypairs_num(j,2))))*(sqrt(pop_citypairs(j,1)*pop_citypairs(j,2)))*1*capacity_seg*0.08+punish1*(-1000000)+punish2*(-1000000);
                    ...
                    for k=1:rand_path% 随机搜索路径，并减少相应容量
                        ...
                        if rand_weight>0
                            ...
                            if path_delete==0 && weight~=0%路径可用，路径所有路段容量减少，保存路径，并进行价值计算
                                ...
                                % 价值计算
                                fitness_value(i)=fitness_value(i)+(sqrt(gdp_citypairs(citypairs_num(j,1))*gdp_citypairs(citypairs_num(j,2))))*(sqrt(pop_citypairs(j,1)*pop_citypairs(j,2)))*weight*capacity_seg*0.08+punish1*(-1000000)+punish2*(-1000000);
